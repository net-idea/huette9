...
This guide covers installing Docker and running the huette9 project with Docker Compose. Use Docker if you want a reproducible environment without installing PHP/Node locally.
...
```bash
docker compose -p huette9 -f docker-compose.yaml up -d --build
```
...
```bash
docker compose -p huette9 -f docker-compose.yaml -f docker-compose.mariadb.yml up -d --build
```
...
```bash
docker compose -p huette9 -f docker-compose.yaml -f docker-compose.mariadb.yml -f docker-compose.adminer.yml -f docker-compose.phpmyadmin.yml up -d --build
```
...
```bash
docker compose -p huette9 -f docker-compose.yaml -f docker-compose.mariadb.yml -f docker-compose.mariadb.dev.yml up -d --build
```
...
```bash
docker compose -p huette9 -f docker-compose.yaml -f docker-compose.redis.yml up -d
```
...
```bash
docker compose -p huette9 -f docker-compose.yaml -f docker-compose.memcache.yml up -d
```
...
```bash
docker compose -p huette9 -f docker-compose.yaml -f docker-compose.postgresql.yml up -d
```

Update your `.env.local`:
```env
DATABASE_URL="postgresql://huette9:nopassword@postgres:5432/huette9?serverVersion=16&charset=utf8"
```
...
DATABASE_URL="mysql://huette9:nopassword@database:3306/huette9?serverVersion=10.11.2-MariaDB&charset=utf8mb4"
...
docker compose -p huette9 ps

# View logs
docker compose -p huette9 logs -f

# View logs for specific service
docker compose -p huette9 logs -f php
docker compose -p huette9 logs -f nginx
docker compose -p huette9 logs -f database

# Stop all containers
docker compose -p huette9 down

# Stop and remove volumes (WARNING: deletes database data)
docker compose -p huette9 down -v

# Restart a specific service
docker compose -p huette9 restart php
```
...
```bash
# PHP container
docker compose -p huette9 exec php bash
docker compose -p huette9 exec php php bin/console cache:clear
docker compose -p huette9 exec php composer install

# Node container
docker compose -p huette9 run --rm node yarn install
docker compose -p huette9 run --rm node yarn build

# Database container
docker compose -p huette9 exec database mysql -u huette9 -pnopassword huette9
docker compose -p huette9 exec database mysqldump -u huette9 -pnopassword huette9 > backup.sql
```
...
docker compose -p huette9 build php

# Force rebuild all containers
docker compose -p huette9 up -d --build --force-recreate

# Pull latest images
docker compose -p huette9 pull
```
...
docker compose -p huette9 exec database mysqladmin ping -h"database" --silent

# View database logs
docker compose -p huette9 logs database
```
...
docker compose -p huette9 exec -u root php chown -R www-data:www-data /var/www/html/var
```
...
docker compose -p huette9 run --rm node rm -rf node_modules
docker compose -p huette9 run --rm node yarn install
```
...
docker compose -p huette9 -f docker-compose.yaml -f docker-compose.mariadb.yml up -d
```
